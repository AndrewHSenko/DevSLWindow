...
def create_sheet(workbook_name, s_name, data):
    ...
    NUM_ROWS = 13 # Changed for 9am start #
    NUM_COLS = 14
    ...
    for col in SHEET.iter_cols(min_col = 3, max_col = 14):
        ...
        end_min = minute + 5
    # For row filling
    hour = 9 # Changed for 9am start #
    PRODS = data
    prods_i = 0
    for row in SHEET.iter_rows(min_row = 2, max_row = 12): # Changed for 9am start #
        # Handles row headers
        row_header = row[0]
        if hour >= 9 and hour < 12: # Changed for 9am start #
